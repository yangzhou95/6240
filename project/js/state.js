initKG = function (data, config, container) {
    ...
    links.forEach(function (link) {
        ...
        nodes[link.source.id1] = link.source
        link.target = nodeDict[link.target]
        nodes[link.target.id1] = link.target
    });
    ...
    if (!config.nodeColor || config.nodeColor === "")
        var defaultNodeColor = [
            ...
            { fill: "rgb(199, 252, 251)", stroke: "#23b3d7", text: "rgb(93, 76, 93)" },
            ...
            { fill: "rgb(240, 230, 140)", stroke: "rgb(61, 60, 52)",text:"rgb(93, 76, 93)"},
            { fill: "rgb(167, 162, 245)", stroke: "rgb(68, 66, 102)",text:"rgb(93, 76, 93)"},
            { fill: "rgb(247, 183, 134)", stroke: "rgb(230, 123, 41)",text:"rgb(93, 76, 93)"}
        ]
    else
    ...
    var width = config.width? config.width:1920,
        height = config.height? config.height:820;
    ...
    var marker =
        ...
        svg.append("marker")
            .attr("id", "end")
            .data(force.nodes())//箭头id，用于其他标记进行引用时的url
    ...
    var edges_line = svg.append("g").selectAll(".edgepath")
        ...
        .attr("marker-end", "url(#end)");//根据箭头标记的id号引用箭头
    ...
    var circle = svg.append("g")
        ...
        .on("click", function (node) {
            ...
            edges_line.style("stroke-width", function (line) {
                //当与连接点连接时变粗
                if ((line.source.id1 == node.id1 || line.target.id1 == node.id1) ) {
            ...
            lastFocusNode = node;
            console.log(lastFocusNode.name)
            document.getElementById("nodes_state").value = lastFocusNode.name;
            document.getElementById("nodes").value = lastFocusNode.name;
            document.getElementById("nodes_social").value = lastFocusNode.name;
        ...
        .on("mouseover",function(node){
            ...
            var content = lastFocusNode.type;
            if (content == "Symptoms") {
                content = lastFocusNode.Symptom
            }else if (content == "Location") {
                content = "Location".bold().concat(": ",lastFocusNode.fullname,"; ","Address: ".bold(),lastFocusNode.address,"; ","vaccine_url: ".bold(),lastFocusNode.vaccine_url,"; ","State: ".bold(),lastFocusNode.State)
            }else if (content  == "States") {  
                content = "States: ".bold().concat(lastFocusNode.name)
            }else if (content == "Side_effect") {
                content = "Side effect: ".bold().concat(lastFocusNode.Side_effect_id,"; ","Body parts:".bold(),lastFocusNode.Body_parts)
            }else if (content =="Vaccine_info") {
                content = "Target_people: ".bold().concat(lastFocusNode.Target_people,"; ","Type: ".bold(),lastFocusNode.Type,"; ","Trial_participants: ".bold(),lastFocusNode.Trial_participants,"; ","Cost_per_dose: ".bold(),lastFocusNode.Cost_per_dose,"; ","Vaccine_id: ".bold(),lastFocusNode.Vaccine_id,"; ","Manufacturer: ".bold(),lastFocusNode.Manufacturer,"; ","Efficacy: ".bold(),lastFocusNode.Efficacy,"; ","How_given: ".bold(),lastFocusNode.How_given)
            }else if (content == "node"||content == "main") {
                content = lastFocusNode.name
            }else if (content == "MYTH"||content =="FACTS") {
                content = lastFocusNode.type.bold().concat(": ".bold(),lastFocusNode.Social_Media)
            }else if (content == "Symptoms_withtreatment") {
                content = lastFocusNode.Symptom
            }else if (content == "treatment") {
                content = lastFocusNode.Treatment
            }
              
            
  
              
              document.querySelector('#boxf1').innerHTML = content;
        })
